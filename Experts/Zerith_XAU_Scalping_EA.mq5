...
//|  Decision Matrix & Adaptive Multi-Layer Recovery (v17.00)        |
//|  FIX: Basket-level win rate, cooldown, profit factor, pending    |
//+------------------------------------------------------------------+
...
#property description "Zerith XAU Scalping MT5 v17.00 - Decision Matrix & Adaptive Multi-Layer Recovery"
#property strict
...
input group ">>>> 1. Zerith XAU Scalping: Trading Basics"
...
input group ">>>> 4. Zerith XAU Scalping: Recovery Grid Spacing"
input int      InpRecoveryMinGap      = 80;           // Minimum Recovery Gap (Points)
input double   InpGapAtrMult          = 1.5;          // Dynamic Gap ATR Multiplier
...
void ManageRecovery(TFView &views[], bool &viewsOk[])
{
   ...
   for(int direction=0; direction<=1; direction++)
   {
      ...
      double atr_gap = (g_atrM1Pts > 0) ? g_atrM1Pts * InpGapAtrMult : InpRecoveryMinGap;
      double min_gap = MathMax((double)InpRecoveryMinGap, atr_gap);
...
void DashInit()
{
   ...
   ObjLabel(DASH_PFX+"title", x+12,y+5,  "ZERITH XAU SCALPING v17.00",   CLR_TITLE, 10, DASH_FONT_BOLD);
